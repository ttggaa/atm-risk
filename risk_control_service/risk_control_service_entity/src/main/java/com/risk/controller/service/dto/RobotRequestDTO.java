...
public class RobotRequestDTO {
    ...
    private Integer userDeviceSmsSensitiveWordCount;//短信验证-一般敏感词个数
    private Integer userDeviceCallRecordBlackList;//用户设备通话记录黑名单个数
    ...
    private Integer userOpertorRealnameCheck;//运营商实名认证状态
    private Integer userDeviceContactUsedCount;//设备有效通讯录个数（在运营商通话记录有通话过，算有效）
    private Integer userOpertorPhoneUsedTime;//手机使用时长
    private Integer userDeviceAndOperatorCount;//通讯录与运营商通通话次数
    private Integer userOperatorAvgCharge;// 运营商平均话费
    private Integer userMainContactNum;//紧急联系人通话次数
    private Integer userMainContactTime;//紧急联系人通话时长
    private Integer userShumeiCount;//树美多头借贷个数
    private Integer userShumeiRejectCount;//树美在多个不同网贷平台被拒绝
