...
public class RSAUtils {
	 ...
	 * @throws Exception
	 */
	public static String decrypt(PrivateKey pk, String content) throws Exception {
		try {
	...
	public static String decrypt(byte[] string,PrivateKey privateKey) {
		try {
			Security.addProvider(new org.bouncycastle.jce.provider.BouncyCastleProvider());
			Cipher cipher = Cipher.getInstance("RSA/None/PKCS1Padding", "BC");
			cipher.init(Cipher.DECRYPT_MODE, privateKey);
			byte[] plainText = cipher.doFinal(string);
			String s = Base64.encode(plainText);
			return s;
		}catch (Exception e) {
			throw new RuntimeException(e);
		}
